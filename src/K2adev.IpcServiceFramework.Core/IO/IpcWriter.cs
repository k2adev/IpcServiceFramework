...
namespace K2adev.IpcServiceFramework.IO
{
    public class IpcWriter : IDisposable
    {
        ...
        public IpcWriter(Stream stream, IIpcMessageSerializer serializer, IpcServiceOptions options)
            : this(stream, serializer, options, leaveOpen: false)
        { }

        public IpcWriter(Stream stream, IIpcMessageSerializer serializer, IpcServiceOptions options, bool leaveOpen)
        {
            _stream = stream;
