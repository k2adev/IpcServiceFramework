...
namespace K2adev.IpcServiceFramework.IO
{
    public class IpcReader : IDisposable
    {
        ...
        public IpcReader(Stream stream, IIpcMessageSerializer serializer, IpcServiceOptions options)
            : this(stream, serializer, options, leaveOpen: false)
        { }

        public IpcReader(Stream stream, IIpcMessageSerializer serializer, IpcServiceOptions options, bool leaveOpen)
        {
            _stream = stream;
            _serializer = serializer;
            _serializer.Options = options;
